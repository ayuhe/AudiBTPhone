...
public class SupportActivity extends AppCompatActivity implements ISupport{
				...
				public void onBackPressed(){
								...
								if(mFragmentation.dispatchBackPressedEvent(activeFragment)){
												if(activeFragment.onActivityBackPressed()){ // 当前Activity可Finish
																finish();
												}
												return;
								}
				...
				public void startForResult(SupportFragment toFragment, int requestCode){
								mFragmentation.dispatchStartTransaction(getSupportFragmentManager(), getTopFragment(), toFragment, requestCode, SupportFragment.STANDARD,
								                                        Fragmentation.TYPE_ADD_RESULT, null, null);
				}

				@Override
				public void startWithPop(SupportFragment toFragment){
								mFragmentation.dispatchStartTransaction(getSupportFragmentManager(), getTopFragment(), toFragment, 0, SupportFragment.STANDARD,
