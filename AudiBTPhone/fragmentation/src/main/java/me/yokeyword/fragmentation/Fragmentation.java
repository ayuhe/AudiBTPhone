...
public class Fragmentation{
				...
				/**
					...
					* @param type        类型
					*/
				void dispatchStartTransaction(FragmentManager fragmentManager, SupportFragment from, SupportFragment to, int requestCode, int launchMode, int
												type, View sharedElement, String name){
				...
				SupportFragment getActiveFragment(SupportFragment parentFragment, FragmentManager fragmentManager){
								...
								for(int i = fragmentList.size() - 1; i >= 0; i--){
												...
												if(fragment instanceof SupportFragment){
																...
																// 只有当fragment在Right的位置时，才处理返回按键处理
																if(supportFragment.getLayoutGravity().equals(SupportFragment.LayoutGravity.Main)){
				...
				/**
					* 判断是否需要延时处理
					* 解决同时按返回键太快导致弹栈两次，把两个fragment弹出的bug
					* @return
					*/
												...
												if(fragment instanceof SupportFragment){
																...
																if(!flag){
																				...
																				mHandler.postDelayed(new Runnable(){
																								@Override
																								public void run(){
																												supportFragment.onFragmentResult(finalFragmentResultRecord.requestCode, finalFragmentResultRecord.resultCode,
                                    finalFragmentResultRecord.resultBundle);
																								}
				...
				private void handlePopAnim(SupportFragment fromFragment, View view, SupportFragment toFragment){
								try{
												if(view != null){
																...
																if(toFragment == null && Build.VERSION.SDK_INT < 21){
